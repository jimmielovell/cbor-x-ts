import { Decoder, mult10, addExtension as decodeAddExtension } from './decode';
...
export class Encoder extends Decoder {
	...
	constructor(options) {
		...
		let encodeUtf8 = (textEncoder && textEncoder.encodeInto)
			? function(string: string, position: number) {
		...
		this.encode = function(value: any, encodeOptions) {
			if (!target) {
...
function writeArrayHeader(length: number) {
	if (length < 0x18) {
		target[position++] = 0x80 | length;
	} else if (length < 0x100) {
...
function findRepetitiveStrings(value: string | [] | {}, packedValues) {
...
extensions = [{
	...
	encode(set: [], encode: Function) {
		let array = Array.from(set)
		encode(array)
	}
}, {
	tag: 27, // http://cbor.schmorp.de/generic-object
	encode(error: Error, encode: Function) {
		encode([ error.name, error.message ])
	}
}, {
	tag: 27, // http://cbor.schmorp.de/generic-object
	encode(regex: RegExp, encode: Function) {
		encode([ 'RegExp', regex.source, regex.flags ])
	}
}, {
	encode(arrayBuffer: Buffer, encode, makeRoom: Function) {
...
function writeBuffer(buffer: Buffer, makeRoom: Function) {
